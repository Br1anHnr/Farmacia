...
export class AgentBotService {
   ...
   */
  public deactivateBotForConversation(
    conversationId: number,
    reason: string,
  ): void {
    this.conversationStates.set(conversationId, "HUMAN_ACTIVE");
    console.log(
      `[AgentBot] Bot desativado atomicamente para conversa #${conversationId}. Motivo: ${reason}`,
    );
  }
  ...
  public async handleIncomingMessage(
    ...
    const isPrescriptionMention =
      content.includes("receita") ||
      content.includes("médico") ||
      content.includes("prescrição") ||
      content.includes("receituário");
    if (hasAttachments || isPrescriptionMention) {
      // Confirmação cordial sem diagnóstico clínico e handoff imediato
      const botMessage =
        "👋 Olá! Recebemos sua receita médica com sucesso.\nNossa equipe farmacêutica já está verificando os medicamentos para te passar os valores e a disponibilidade! 🛵💨";

      if (
        !(await chatwootAdapter.sendMessage(conversationId, botMessage)).success
      )
        throw new Error("BOT_MESSAGE_FAILED");
      if (!(await chatwootAdapter.updateStatus(conversationId, "open")))
        throw new Error("BOT_STATUS_FAILED");
      if (
        !(await chatwootAdapter.addLabels(conversationId, [
          "receita-medica",
          "farmaceutico",
        ]))
      )
        throw new Error("BOT_LABELS_FAILED");
      this.deactivateBotForConversation(
        conversationId,
    ...
    if (
      ...
      const botMessage =
        "Entendido! Estou transferindo agora mesmo para um de nossos atendentes humanos. Um momento, por favor!";
      if (
        !(await chatwootAdapter.sendMessage(conversationId, botMessage)).success
      )
        throw new Error("BOT_MESSAGE_FAILED");
    ...
    if (
      ...
      (content.includes("entrega") &&
        (content.includes("quando") ||
          content.includes("saiu") ||
          content.includes("atrasad") ||
          content.includes("status")))
    ) {
      const botMessage =
        "Você gostaria de consultar o status da sua entrega? Já chamei um atendente para localizar seu pedido!";
      if (
        !(await chatwootAdapter.sendMessage(conversationId, botMessage)).success
      )
        throw new Error("BOT_MESSAGE_FAILED");
